...
struct Lidar
{
	...
	{
		// COMPLETED:: set minDistance to 5 to remove points from roof of ego car
		minDistance = 5; //meters
		maxDistance = 50;
		resoultion = 0.2;
		// COMPLETED:: set sderr to 0.2 to get more interesting pcd files
		sderr = 0.2;
		cars = setCars;
		groundSlope = setGroundSlope;

		// COMPLETED:: increase number of layers to 8 to get higher resoultion pcd
		int numLayers = 8;
		// the steepest vertical angle
		double steepestAngle =  30.0*(-pi/180);
		double angleRange = 26.0*(pi/180);
		// COMPLETED:: set to pi/64 to get higher resoultion pcd
